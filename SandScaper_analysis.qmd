...
```{r}
summary(factor(data_long$gender))
```
...
data_long %>% select(id, system, ueq_cols) %>%
  pivot_longer(., cols = ueq_cols, names_to = "Var", values_to = "Val") %>%
  my_ueq_boxplot(
    x = "Var",
    y = "Val",
    group = "system",
  ...
  # ggplot(aes(x = Var, y = Val, fill = system)) +
  # geom_boxplot()
